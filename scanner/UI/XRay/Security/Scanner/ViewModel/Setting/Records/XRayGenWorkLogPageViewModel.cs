...
namespace UI.XRay.Security.Scanner.ViewModel.Setting.Records
{
    ...
    /// 射线源工作日志查询视图模型
    /// </summary>
    public class XRayGenWorkLogPageViewModel : PageViewModelBase
    {
        /// <summary>
        /// 检索周期选择变化更改事件
        /// </summary>
        ...
        private XRayGenWorkLogsRetrievalController _controller;
        ...
        public XRayGenWorkLogPageViewModel()
        {
            _controller = new XRayGenWorkLogsRetrievalController();
        ...
        private void FindCommandExecute()
        {
            ...
            switch (SelectedPeriod)
            {
                case StatisticalPeriod.Dayly:
                    new OperationRecordService().RecordOperation(OperationUI.XRayGenWorkLog, string.Format("{0}-{1}", SelectedYear, SelectedMonth), OperationCommand.Find, "Dayly");
                    break;
                case StatisticalPeriod.Weekly:
                    new OperationRecordService().RecordOperation(OperationUI.XRayGenWorkLog, string.Format("{0}", SelectedYear), OperationCommand.Find, "Weekly");
                    break;
                case StatisticalPeriod.Monthly:
                    new OperationRecordService().RecordOperation(OperationUI.XRayGenWorkLog, string.Format("{0}", SelectedYear), OperationCommand.Find, "Monthly");
                    break;
                case StatisticalPeriod.Quarterly:
                    new OperationRecordService().RecordOperation(OperationUI.XRayGenWorkLog, string.Format("{0}", SelectedYear), OperationCommand.Find, "Quarterly");
                    break;
                default:
                    break;
            }
        ...
        private void ExportCommandExecute()
        {
            var msg = new ShowFolderBrowserDialogMessageAction("SettingWindow", s =>
            {
                if (_controller != null && !string.IsNullOrWhiteSpace(s))
                {
                    ...
                    }, ConfigHelper.GetExportFileName(OperationUI.XRayGenWorkLog, s, TranslationService.FindTranslation("XRayGenWorkLog")));
