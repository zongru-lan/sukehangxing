...
namespace UI.XRay.Gui.Framework
{
    ...
    public class AccountGroupMessage : MessageBase
    {
        public AccountGroupMessage(AccountGroup group)
        {
            Group = group;
        }
    ...
    public class TipLibSelectMessage : MessageBase
    {
        public TipLibSelectMessage(TipPlan tipPlan)
        {
            TipPlan = tipPlan;
        }

        public TipPlan TipPlan { get; set; }
    }

    /// <summary>
    /// 清理异步已选取的Tip图像
    /// </summary>
    public class ClearUpTipSelectedMessage : MessageBase
    {
        public ClearUpTipSelectedMessage()
        {
    ...
    /// 将选好的TIp图像入库
    /// </summary>
    public class PlanandImageToDbMessage : MessageBase
    {
        public PlanandImageToDbMessage()
        {
    ...
    /// 显示Tip图像选择窗口
    /// </summary>
    public class ShowTipImageSelectWindowAction : NotificationMessageAction<TipPlan>
    {
        /// <summary>
        /// 宿主窗口，将作为新打开窗口的所有者Owner
        /// </summary>
        ...
        public ShowTipImageSelectWindowAction(string parentWindowKey, Action<TipPlan> action)
            : base(null, action)
        {
            ParentWindowKey = parentWindowKey;
        }
    ...
    public class ShowOpenFilesDialogMessageAction : NotificationMessageAction<string[]>
    {
        ...
        public bool MultiSelected { get; private set; }

        public ShowOpenFilesDialogMessageAction(string parentWindowKey, string filter, bool multiselected, Action<string[]> action)
            : base(null, action)
        {
            Filter = filter;
    ...
    public class ShowAddGroupWindowAction : NotificationMessageAction<AccountGroup>
    {
        public string ParentWindowKey { get; private set; }

        public ShowAddGroupWindowAction(string parentWindowKey, Action<AccountGroup> action)
            : base(null, action)
        {
            ParentWindowKey = parentWindowKey;
        }
    ...
    /// 触发急停事件
    /// </summary>
    public class EmergencyMessage : MessageBase
    {
    ...
    public class ValidationResultMessage : MessageBase
    {
        public string TargetName { get; set; }
        public ValidationResult Result { get; set; }

        public ValidationResultMessage(string target, ValidationResult result)
        {
            TargetName = target;
            Result = result;
        }
    }
    public class LoadKeyFunctionMessage : MessageBase
    {
        public LoadKeyFunctionMessage()
        {

        }
    }

    public class WorkReminderChangedMessage : MessageBase
    {
        public TimeIntervalEnum IntervalEnum;

        public bool IsShifts;

        public WorkReminderChangedMessage(TimeIntervalEnum intervalEnum, bool isShifts = true)
        {
