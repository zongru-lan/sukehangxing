...
namespace UI.XRay.Flows.Services
{
    ...
    /// 此类不再使用：闪灯报警交由控制板完成
    /// </summary>
    public class XRayLampControlService
    {
        /// <summary>
        ...
        /// 单实例服务
        /// </summary>
        public static XRayLampControlService Service { get; private set; }

        static XRayLampControlService()
        {
            Service = new XRayLampControlService();
        }

        protected XRayLampControlService()
        {
        }
        ...
        private void ServicePartOnScannerWorkingStatesUpdated(object sender, ScannerWorkingStates scannerWorkingStates)
        {
            lock (this)
            {
                ...
                if (_isXRayOn != xrayOn && _flareDesc == null)
                {
                    // 射线状态发生变化，并且当前未闪烁报警，则更新射线状态
                    ControlService.ServicePart.LightXRayLamp(xrayOn);
                }
                _isXRayOn = xrayOn;
            }
        }
                ...
                while (times > 0)
                {
                    lock (this)
                    {
                        ...
                        if (times == 0)
                        {
                            ...
                            // 还原X射线指示灯状态至最新
                            ControlService.ServicePart.LightXRayLamp(_isXRayOn);

                            break;
                        }
                    }

                    ControlService.ServicePart.LightXRayLamp(true);
                    Thread.Sleep(pulse);

                    ControlService.ServicePart.LightXRayLamp(false);
    ...
    class FlareDesc
    {
        public FlareDesc(TimeSpan pulse, TimeSpan interval, int times)
        {
            PulseWidth = pulse;
            Interval = interval;
            Times = times;
