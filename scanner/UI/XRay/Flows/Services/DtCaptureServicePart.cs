...
namespace UI.XRay.Flows.Services
{
    ...
    /// 基于DT的图像采集系统
    /// </summary>
    public class DtCaptureServicePart : ICaptureServicePart
    {
        private DetSysTypeEnum _dtDetSysType;

        private DTControlClr _dtControl;
        ...
        public DtCaptureServicePart()
        {
            Tracer.TraceEnterFunc("UI.XRay.Flows.Services.DtCaptureServicePart");

            IntegrationTime = 4000;

            try
            {
                _dtControl = new DTControlClr();
            ...
            {
                Tracer.TraceException(exception, "Unexpected exception in DtCaptureServicePart constructor.");
            }

            Tracer.TraceExitFunc("UI.XRay.Flows.Services.DtCaptureServicePart");
        }
        ...
        private void LoadSettings()
        {
            ...
            // 加载启用的探测视角个数
            if (!ScannerConfig.Read(ConfigPath.CaptureSysBoardCount, out _viewsCount))
            {
                _viewsCount = 1;
            }
            ...
            if (view1Cards != null)
            {
                View1Desc = new DTCaptureDesc(view1Cards[0], view1Cards[1], view1Cards[2], view1Cards[3], dualEnergy, 160);
            }
            else
            {
                View1Desc = new DTCaptureDesc(2, 0, 0, 0, dualEnergy, 160);
            }
            ...
            if (IsDualView)
            {
                ...
                if (view2Cards != null)
                {
                    View2Desc = new DTCaptureDesc(view2Cards[0], view2Cards[1], view2Cards[2], view2Cards[3], dualEnergy, 160);
                }
                else
                {
        ...
        private void TryOutputDualViewData()
        {
            ...
            //if (Monitor.TryEnter(this, 10))
            {
                try
                {
                    ...
                    ////todo：test 高琦：2016.11.16 17：08  根据DTCapture部分的逻辑，DT每一帧数据有32线，这样如果有2帧数据不同步，就会造成丢数据，暂时改变此处对数据阈值60为32*8+1 = 257
                    //// 保护机制：防止某个视角没有数据，导致内存泄露
                    //while (View1LinesCache.Count >= 257 && View2LinesCache.Count == 0)
                    //{
                    //    RawScanlineData scanline;
                    //    View1LinesCache.TryDequeue(out scanline);

                    //    if (!isLogDumpdata)
                    //    {
                    //        isLogDumpdata = true;
                    //        //Tracer.TraceWarning("CaptureSys: View1LinesCache.count >= 60 && View2LinesCache.Count == 0, dump view1LinesCache!");
                    //        Tracer.TraceWarning("CaptureSys: View1LinesCache.count >= 257 && View2LinesCache.Count == 0, dump view1LinesCache!");
                    //    }
                    //}

                    //isLogDumpdata = false;

                    //while (View2LinesCache.Count >= 257 && View1LinesCache.Count == 0)
                    //{
                    //    RawScanlineData scanline;
                    //    View2LinesCache.TryDequeue(out scanline);

                    //    if (!isLogDumpdata)
                    //    {
                    //        isLogDumpdata = true;
                    //        //Tracer.TraceWarning("CaptureSys: View2LinesCache.count >= 60 && View1LinesCache.Count == 0, dump view2LinesCache!");
                    //        Tracer.TraceWarning("CaptureSys: View2LinesCache.count >= 257 && View1LinesCache.Count == 0, dump view2LinesCache!");
                    //    }
                    //}
                }
                catch (Exception e)
        ...
        public bool Open()
        {
            IsOpened = _dtControl.Open(ViewsCount, (DTDetSysType)_dtDetSysType);
            if (IsOpened)
            {
                IntegrationTime = _dtControl.IntegrationTime;
                _dtControl.ScanlineCaptured += DtControlOnScanlineCaptured;
        ...
        {
            return _dtControl.StartCapture();
        }

        [MethodImpl(MethodImplOptions.Synchronized)]
        public void StopCapture()
        {
            _dtControl.StopCapture();
        }
        public void SetIntegrationTime(float integrationTime)
        {
            _dtControl.SetIntegrationTime((int)integrationTime);
        }
